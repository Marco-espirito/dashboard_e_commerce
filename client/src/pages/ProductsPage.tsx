...
const EMPTY_FORM: ProductForm = { name: "", price: "", stock: "", category: "" };
const PER_PAGE = 8;
...
export function ProductsPage() {
  ...
  const stockFilter: "low" | "all" = searchParams.get("stock") === "low" ? "low" : "all";
  ...
  const productsParams = { page: currentPage, limit: PER_PAGE, stock: stockFilter };

  const { data, isLoading: loading, error } = useQuery({
    queryKey: queryKeys.products(productsParams),
    queryFn: () => fetchProducts(productsParams),
  });

  const products = data?.products ?? [];
  const total = data?.total ?? 0;
  const totalPages = data?.totalPages ?? 1;
  const stats = data?.stats ?? { totalOrdered: 0, totalCurrentStock: 0, totalBeforePurchases: 0 };
  ...
  const saveMutation = useMutation({
    mutationFn: (body: { name: string; price: number; stock: number; category: string | null }) =>
      editingId
        ? api(`/products/${editingId}`, { method: "PATCH", body })
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { setProductToDelete(null); invalidate(); },
  ...
  function handleSubmit(e: FormEvent) {
    ...
    if (!Number.isFinite(price) || price < 0) { setFormError("Prix invalide"); return; }
    if (!Number.isInteger(stock) || stock < 0) { setFormError("Stock invalide"); return; }
    saveMutation.mutate({ name: form.name, price, stock, category: form.category || null });
  }
  ...
  return (
    <div>
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">Ajoute, modifie et gere le stock de ta boutique.</p>
      </div>

      {stockFilter === "low" && (
        <div className="mt-6 flex flex-wrap items-center gap-3 rounded-xl border border-amber-100 bg-amber-50 px-4 py-3">
          <span className="text-sm font-medium text-amber-800">Filtre actif : produits en stock faible</span>
      ...
      <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <div className="text-xs font-medium uppercase tracking-wide text-slate-400">Articles achetes</div>
          <div className="mt-2 text-2xl font-semibold text-slate-900">{stats.totalOrdered}</div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <div className="text-xs font-medium uppercase tracking-wide text-slate-400">Stock actuel</div>
          <div className="mt-2 text-2xl font-semibold text-slate-900">{stats.totalCurrentStock}</div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <div className="text-xs font-medium uppercase tracking-wide text-slate-400">Stock avant achat</div>
          <div className="mt-2 text-2xl font-semibold text-slate-900">{stats.totalBeforePurchases}</div>
        </div>
      </div>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[360px_1fr]">
        <section className="h-fit rounded-2xl border border-slate-200 bg-white p-6">
          ...
          <form onSubmit={handleSubmit} className="mt-4 space-y-3">
            ...
            <input type="number" min="0" step="0.01" value={form.price}
              onChange={(e) => updateForm("price", e.target.value)} required placeholder="Prix en euros"
              className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" />
            <input type="number" min="0" step="1" value={form.stock}
              onChange={(e) => updateForm("stock", e.target.value)} required placeholder="Stock"
              className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" />
            <input value={form.category} onChange={(e) => updateForm("category", e.target.value)} placeholder="Categorie"
              className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" />
            ...
            <div className="flex gap-2">
              <button type="submit" disabled={saveMutation.isPending}
                ...
                {saveMutation.isPending ? "Enregistrement..." : editingId ? "Enregistrer" : "Ajouter"}
              </button>
              {editingId && (
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
          {loading ? (
            ...
            <p className="px-6 py-8 text-center text-sm text-slate-400">Aucun produit pour l'instant.</p>
          ) : (
              ...
              <tbody className="divide-y divide-slate-100">
                {products.map((product) => (
                  <tr key={product.id}>
                    ...
                    <td className="px-6 py-3 text-slate-700">{product.stockBeforePurchases}</td>
                    <td className="px-6 py-3 text-right">
                      <div className="flex justify-end gap-3">
                        ...
                        <button type="button" onClick={() => setProductToDelete(product)}
                          ...
                          title={product._count.orderItems > 0 ? "Produit utilise dans des commandes" : "Supprimer"}
      ...
      {totalPages > 1 && (
        <div className="mt-4 flex items-center justify-end gap-3">
          ...
          <button type="button" onClick={() => setParams({ page: String(currentPage - 1) })}
      ...
      {productToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <h2 className="text-lg font-semibold text-slate-900">Supprimer le produit ?</h2>
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button type="button" onClick={() => deleteMutation.mutate(productToDelete.id)}
